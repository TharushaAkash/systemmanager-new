...
@Component
public class FuelOnlyInvoiceWorkflow extends InvoiceCreationTemplate {
    ...
    protected void validateSource(Long bookingId) {
        ...
        if (booking.getLitersRequested() == null || booking.getLitersRequested() <= 0 || booking.getFuelType() == null) {
            throw new IllegalArgumentException("No fuel request found in booking: " + bookingId);
        }
    ...
    protected void afterPersist(Invoice invoice, Long bookingId) {
        ...
        debitEntry.setDescription("Fuel-only invoice created for booking " + bookingId);
        ...
        creditEntry.setAccount("FUEL_REVENUE");
        ...
        creditEntry.setDescription("Fuel revenue from invoice " + invoice.getInvoiceNumber());
